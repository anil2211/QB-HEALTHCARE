...
app.get('/api/patients/:id/records', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT r.id, r.note, r.created_at, d.full_name AS doctor FROM records r LEFT JOIN doctors d ON d.id = r.doctor_id WHERE r.patient_id = $1 ORDER BY r.created_at DESC',
      [id]
    );
    res.json(rows);
  } catch (err) {
...
app.get('/api/appointments', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT a.id, a.starts_at, a.ends_at, a.reason, a.status,
...
app.post('/api/appointments', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      'INSERT INTO appointments (patient_id, doctor_id, starts_at, ends_at, reason) VALUES ($1,$2,$3,$4,$5) RETURNING *',
      [patient_id, doctor_id, starts_at, ends_at, reason]
    );
